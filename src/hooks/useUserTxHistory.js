...
export const useUserTxHistory = account => {
  ...
  useAsyncEffect(async () => {
    try {
      ...
      // const arbTxHistoryData = await axios.post(
      ...
      // const bnbTxHistoryData = await axios.post(
      ...
      // const arbTxHistory = [];
      // if (arbTxHistoryData.data.data.users.length > 0) {
      //   (arbTxHistoryData.data.data.users[0].deposits || []).forEach(item => {
      //     arbTxHistory.push({
      //       ...item,
      //       chain:
      ...
      //           : 'Arbitrum Test',
      //       plan: Number(item.plan) === 0 ? 'Vesting 1' : 'Vesting 2',
      //       txHash: `${CHAINS[CHAIN_IDS[1]].blockExplorerUrls}tx/${
      //         item.transactionHash
      //       }`,
      //       amount: getReadableNumber(
      //         item.amount,
      //         getAssetDecimal(item.token, CHAIN_IDS[1])
      //       ),
      //       asset: getAssetName(item.token, CHAIN_IDS[1]),
      //       boughtAmount: getReadableNumber(item.boughtAmount, 36),
      //       date: new Date(item.blockTimestamp * 1000).toLocaleString('en', {
      //         year: 'numeric',
      //         month: '2-digit',
      ...
      //   (arbTxHistoryData.data.data.users[0].harvests || []).forEach(item => {
      //     arbTxHistory.push({
      //       ...item,
      //       chain:
      //         process.env.REACT_APP_ENV === 'prod'
      ...
      //   (arbTxHistoryData.data.data.users[0].releases || []).forEach(item => {
      //     arbTxHistory.push({
      //       ...item,
      //       amount: '',
      //       chain:
      ...
      // const bnbTxHistory = [];
      // if (bnbTxHistoryData.data.data.users.length > 0) {
      //   (bnbTxHistoryData.data.data.users[0].deposits || []).forEach(item => {
      //     bnbTxHistory.push({
      //       ...item,
      //       chain: process.env.REACT_APP_ENV === 'prod' ? 'BSC' : 'BSC Test',
      //       plan: Number(item.plan) === 0 ? 'Vesting 1' : 'Vesting 2',
      //       txHash: `${CHAINS[CHAIN_IDS[2]].blockExplorerUrls}tx/${
      //         item.transactionHash
      //       }`,
      //       amount: getReadableNumber(
      //         item.amount,
      //         getAssetDecimal(item.token, CHAIN_IDS[2])
      //       ),
      //       asset: getAssetName(item.token, CHAIN_IDS[2]),
      //       boughtAmount: getReadableNumber(item.boughtAmount, 36),
      //       date: new Date(item.blockTimestamp * 1000).toLocaleString('en', {
      //         year: 'numeric',
      //         month: '2-digit',
      ...
      //   (bnbTxHistoryData.data.data.users[0].harvests || []).forEach(item => {
      //     bnbTxHistory.push({
      //       ...item,
      //       chain: process.env.REACT_APP_ENV === 'prod' ? 'BSC' : 'BSC Test',
      //       plan: Number(item.plan) === 0 ? 'Vesting 1' : 'Vesting 2',
      ...
      //   (bnbTxHistoryData.data.data.users[0].releases || []).forEach(item => {
      //     bnbTxHistory.push({
      //       ...item,
      //       amount: '',
      //       chain: process.env.REACT_APP_ENV === 'prod' ? 'BSC' : 'BSC Test',

...
function InterestRateModel({ settings, currentAsset, history, marketInfo, getInterateModel }) {
  ...
  const getGraphData = async asset => {
    ...
    const multiplierPerBlock = marketInfo.supplyRatePerBlock || 0;
    const baseRatePerBlock = marketInfo.borrowRatePerBlock || 0;
    const data = [];
  ...
  }, [settings.markets, currentAsset, marketInfo]);
...
const mapDispatchToProps = dispatch => {
  const { getGovernanceStrike, getInterateModel } = accountActionCreators;
